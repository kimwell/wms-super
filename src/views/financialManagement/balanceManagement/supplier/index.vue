      ...
      <div class="card-contnet">
        <div class="table-contnet">
          ...
          <Row v-for="(item,index) in list " :key="index">
            <Col class-name="col" span="6">{{item.buserName}}</Col>
            <Col class-name="col" span="6">{{item.account}}</Col>
            <Col class-name="col" span="6">{{item.waitSettleOrderNum}}</Col>
...
<script>
  export default {
    ...
    computed: {
      handleFilter() {
        return {
          buserId: this.detailApi.buserId,
    ...
    methods: {
      getList() {
        this.$http.post(this.api.buserAccountPage, this.pageApi).then(res => {
          if (res.code === 1000) {
      ...
      openModel(item) {
        this.show = true;
        this.detailApi.buserId = item.id;
        this.getDetail(this.handleFilter);
      },
